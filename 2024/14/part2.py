...
def shouldPrint(robots, width, height):
	row = [0]*width
	rows =[]
	...
	for r in robots:
		rows[r[1]][r[0]] += 1
	for rw in rows:
		stringRow  = ''.join([str(x) for x in rw])
		if re.search(r'[^0]{12}',stringRow):
			return True
	return False
...
def getResult( lines, width, height ):
	...
	for i in range(10000):
		...
		if shouldPrint(robots, width, height):
...
getResult( readData( "input.txt" ), 101,103 )
print( "Done" )